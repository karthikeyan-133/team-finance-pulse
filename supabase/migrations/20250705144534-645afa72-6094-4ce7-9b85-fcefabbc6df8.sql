...
DROP TRIGGER IF EXISTS trigger_update_shop_payments_on_delivery ON public.orders;
...
BEGIN
  ...
  IF NEW.order_status = 'delivered' AND (OLD.order_status IS NULL OR OLD.order_status != 'delivered') THEN
    ...
    RAISE NOTICE 'Updated shop payments for order: %, affected rows: %', NEW.id, ROW_COUNT;
  END IF;
...
-- Recreate trigger on orders table for delivery completion
