...
const CustomerPortal = () => {
  ...
  } = useShops(selectedCategory);
  const {
    products,
    loading: productsLoading
  } = useProducts(selectedShopId, selectedCategory);
  ...
  const fetchOrderHistory = async (customerPhone: string) => {
    try {
      const {
        data: orders,
        error
      } = await supabase.from('orders').select('*').eq('customer_phone', customerPhone).order('created_at', {
        ascending: false
      }).limit(10);
      if (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const {
        ...
        error
      } = await supabase.from('customers').select('*').eq('phone', loginPhone.trim()).single();
  ...
  const handleProductsDisplay = (shopId: string, shopName: string) => {
    ...
    if (shopProducts.length === 0) {
      ...
      if (latestProducts.length === 0) {
        ...
        addBotMessage(`Perfect! Here are the available ${selectedCategory.toLowerCase()} items from ${shopName}. You can add items to your cart by clicking on them:${extraChargeMessage}`, undefined, latestProducts);
      }
    } else {
  ...
  const handleProductAdd = (product: any) => {
    ...
    if (existingItem) {
      setCart(cart.map(item => item.name === product.name ? {
        ...item,
  ...
  return <div className="min-h-screen bg-gray-50 flex flex-col max-w-sm mx-auto">
      ...
      <div className="bg-white shadow-sm border-b px-3 py-2 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <img src="/lovable-uploads/1bce2b84-91de-467d-b543-bc74a5bdffd6.png" alt="Slicker Connect Logo" className="h-8 w-8 flex-shrink-0" />
          ...
          {(shopsLoading || productsLoading) && <Loader2 className="h-3 w-3 animate-spin text-blue-600 flex-shrink-0" />}
        </div>
        
        {/* Cart & User Info */}
        <div className="flex items-center gap-2">
          {/* New Order Button - Show when order is completed */}
          {currentStep === 'completed' && <Button variant="outline" size="sm" onClick={startNewOrder} className="h-6 px-2 text-xs">
              <RotateCcw className="h-3 w-3 mr-1" />
              New Order
            </Button>}
          
          {cart.length > 0 && currentStep !== 'completed' && <div className="flex items-center gap-1 bg-blue-100 px-2 py-1 rounded-full">
              <ShoppingCart className="h-3 w-3 text-blue-600" />
              <span className="text-xs font-medium text-blue-800">
                {getTotalItems()} • ₹{getTotalAmount()}
              </span>
            </div>}
          {customer && <div className="flex items-center gap-1">
              <User className="h-3 w-3 text-gray-600" />
              <Button variant="outline" size="sm" onClick={handleLogout} className="h-6 w-6 p-0">
      ...
      {showLoginForm && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input type="tel" placeholder="Enter your phone number" value={loginPhone} onChange={e => setLoginPhone(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleLogin()} className="text-sm h-9" />
                </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 pb-16">
        {messages.map(message => <ChatMessage key={message.id} message={message} onOptionClick={message.type === 'bot' && message.options ? currentStep === 'welcome' ? handleOptionClick : currentStep === 'category_selection' ? handleCategorySelection : currentStep === 'shop_selection' ? handleShopSelection : handleOptionClick : undefined} onProductAdd={message.products ? handleProductAdd : undefined} />)}
        <div ref={messagesEndRef} />
      </div>

      {/* Input Area - Mobile Optimized */}
      {currentStep !== 'completed' && ['register', 'register_address', 'custom_time_input'].includes(currentStep) && <div className="bg-white border-t p-2 fixed bottom-0 left-0 right-0">
          <div className="flex gap-2 max-w-sm mx-auto">
            <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={e => e.key === 'Enter' && (currentStep === 'custom_time_input' ? handleCustomTimeInput() : handleRegistration())} placeholder={currentStep === 'custom_time_input' ? "e.g., Today 3:00 PM..." : "Type your message..."} className="flex-1 text-sm h-8" />
            <Button onClick={currentStep === 'custom_time_input' ? handleCustomTimeInput : handleRegistration} disabled={!inputValue.trim()} className="h-8 px-2">
